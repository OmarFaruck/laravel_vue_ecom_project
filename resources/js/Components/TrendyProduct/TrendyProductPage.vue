        ...
        <div class="card mt-4 px-4 py-4 container text-center justify-content-center">
            ...
            <Vue3EasyDataTable :headers="headers" :items="items" :rows-per-page="10" border-cell
                ...
                <template #item-image="item">
                    <img :src="`/storage/trendyproducts/${item.image}`" width="80" height="60" class="rounded" />
                </template>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form @submit.prevent="submitUpdate">
                        <div class="modal-header">
                            <h5 class="modal-title">TrendyProducts Edit Form</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
...
const submitcreate = () => {
    form.post("/admin/trendyproducts", {
        onSuccess: () => {
    ...
    swal("Good job!", "This TrendyProducts will be added", "success");
};
...
function submitUpdate() {
    ...
    form.transform((data) => ({
        ...
        .post(`/admin/trendyproducts/${form.id}`, {
            forceFormData: true,
            onFinish: () => {
                form.reset();
                // 🔥 Force remove modal overlay + body lock
...
function remove(data) {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            router.delete(`/admin/trendyproducts/${data.id}`, {
                onSuccess: () => {
